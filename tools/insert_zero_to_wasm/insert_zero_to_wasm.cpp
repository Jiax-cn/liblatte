...
uint8_t *encode_custom_section(const char *name, uint8_t *data, uint32_t data_length, uint32_t *ret_section_length)
{
    ...
    content_length = leb128_encode_uint32_len(name_length) + name_length + data_length;
    section_length = sizeof(wasm_section_idx) + leb128_encode_uint32_len(content_length) + content_length;
...
int main(int argc, char *argv[])
{
    ...
    int f;
    char *f_out = (char *)malloc(strlen(f_in) + 1 + strlen(OUT_PREFIX));
    ...
    uint8_t *data = (uint8_t *)malloc(d_size);
